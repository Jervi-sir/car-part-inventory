...
import { ChevronLeft, ChevronRight, Pencil, Trash, Plus, RefreshCw } from "lucide-react";
...
const csrf =
  (typeof document !== "undefined" && (document.querySelector('meta[name="csrf-token"]') as HTMLMetaElement)?.content) || "";
...
const http = {
  ...
  patch: (url: string, body?: Json) =>
    fetch(url, { method: "PATCH", headers: baseHeaders(), credentials: "same-origin", body: body ? JSON.stringify(body) : undefined }),
  delete: (url: string) =>
...
export default function PartsIndex() {
  ...
  const fetchLookups = async () => {
    const [cRes, mRes] = await Promise.all([http.get(endpoints.categories), http.get(endpoints.manufacturers)]);
  ...
  const bulkSetActive = async (val: boolean) => {
    ...
    await http.post(endpoints.partsBulkStatus, { ids: allSelectedIds.map(Number), is_active: val });
  ...
  const remove = async (row: PartRow) => {
    ...
    await http.delete(endpoints.part(row.id));
    await fetchData(pageData.page);
  };
            ...
            <TableBody>
              ...
              {pageData.data.map((row) => (
                <TableRow key={String(row.id)}>
                  <TableCell>
                    <Checkbox checked={!!selected[String(row.id)]} onCheckedChange={(v) => toggleSelect(row.id, !!v)} />
                  </TableCell>
                  ...
                  <TableCell>
                    <Switch
                      ...
                      onCheckedChange={async (v) => {
                        ...
                        try {
                          const res = await http.patch(endpoints.partActive(row.id), { is_active: v });
