...
import { ChevronLeft, ChevronRight, ShoppingCart, Trash2, Plus, Minus, ChevronDown } from "lucide-react";
...
type PartRow = {
  ...
  category?: { id: Id; name: string } | null; // always null with current schema

  min_order_qty: number;
  min_qty_gros: number;
  price_retail?: number | null;       // TTC mapped on backend
  price_demi_gros?: number | null;    // null (not in schema)
  price_gros?: number | null;         // TTC wholesale mapped on backend

  fitment_models: string[];
  fitment_brands: string[];
  references: { type: string; code: string; source_brand?: string | null }[]; // [] with current schema
};
...
export default function CatalogPage() {
  ...
  const visibleCount = useMemo(() => Object.values(columnVisibility).filter(Boolean).length, [columnVisibility]);
  ...
  useEffect(() => {
    (async () => {
      const [{ data: mj }, { data: bj }] = await Promise.all([api.get(endpoints.mans), api.get(endpoints.vbrands)]);
  ...
  useEffect(() => {
    refreshParts(1);
  }, [filters.manufacturer_id, filters.vehicle_brand_id, filters.vehicle_model_id]);
                ...
                <SelectContent>
                  ...
                  {models.map((m) => (
                    <SelectItem key={m.id} value={String(m.id)}>
                      {m.name}
                      {m.year_from ? ` (${m.year_from}${m.year_to ? `–${m.year_to}` : ""})` : ""}
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
            ...
            <div className="space-y-3 max-h-[50vh] overflow-auto pr-1">
              {cart.items.map((it) => (
                <div key={it.id} className="flex gap-3 items-center">
                  ...
                  <div className="flex items-center gap-1">
                    <Button variant="outline" size="icon" onClick={() => updateQty(it.id, Math.max(1, it.qty - 1))}>
                      <Minus className="h-4 w-4" />
                    </Button>
                    <Input
                      type="number"
                    ...
                    />
                    <Button variant="outline" size="icon" onClick={() => updateQty(it.id, it.qty + 1)}>
                      <Plus className="h-4 w-4" />
                    </Button>
